...
class Amasty_Rules_Model_Observer
{
    ...
    protected function _getSortedCartPices($rule, $address)
    {
        ...
        foreach ($this->_getAllItems($address) as $item) {
            ...
            /**
             * Modifying the rule action "Get $y for each $x spent"
             * so that it looks at the subtotal after discount instead of just the subtotal.
             */
            if ($rule->getSimpleAction() == Amasty_Rules_Helper_Data::TYPE_AMOUNT) {
                $price = $price - $item->getDiscountAmount() / $qty;
                $basePrice = $basePrice - $item->getBaseDiscountAmount() / $qty;
            }
