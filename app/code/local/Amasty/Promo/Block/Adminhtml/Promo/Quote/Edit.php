...
class Amasty_Promo_Block_Adminhtml_Promo_Quote_Edit extends Mage_Adminhtml_Block_Promo_Quote_Edit
{
    public function __construct()
    {
        ...
        $this->_formScripts[] = " 
			...
			function flatrateRegion_hide(){
					
					if ($('rule_set_flatrate_by_region').selectedIndex == 0){
						$('rule_set_flatrate_shipping_us').up().up().show();
						$('rule_set_flatrate_shipping_ca').up().up().show();
					} else {
						$('rule_set_flatrate_shipping_us').up().up().hide();
						$('rule_set_flatrate_shipping_ca').up().up().hide();
					}
				}
				flatrateRegion_hide();
        ";
