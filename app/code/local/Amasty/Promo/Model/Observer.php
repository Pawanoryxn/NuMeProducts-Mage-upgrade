...
class Amasty_Promo_Model_Observer
{
    ...
    protected function _addProductToQuote($quote, $product, $qty, $rule)
    {
        try {
            ...
            $request = $qty;
            /**
             * Configurable products processing
             */
            if ($product->getTypeId() == Mage_Catalog_Model_Product_Type_Configurable::TYPE_CODE) {
                if ($params = Mage::getSingleton('checkout/session')->getData('ampromo_' . $rule->getId() . '_' . $product->getSku())) {
                    ...
                    if (count($allProducts)) {
                        foreach ($allProducts as $simpleProduct) {
                            /**
                             * Finding first available saleable product
                             */
                            if ($simpleProduct->isSaleable()) {
                                $firstProduct = $simpleProduct;
                                break;
                            }
                        }
                        ...
                        foreach ($attributes as $attribute) {
                            ...
                            $superAttribute[$productAttribute->getId()] = $firstProduct->getData($productAttribute->getAttributeCode());
                        }
                        $request = new Varien_Object(array(
