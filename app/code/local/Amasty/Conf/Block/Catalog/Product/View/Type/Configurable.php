        ...
        if ('product.info.options.configurable' == $this->getNameInLayout())
        {
            ...
            if ($this->_optionProducts)
            {
			    ...
			    //'sku'       	=> $simple->getSku(),
                        ...
                        {
                                                        $confData[$strKey]['media_url'] = $this->getUrl('amconf/media', array('id' => $simple->getId(), '_secure' => (Mage::app()->getStore()->isCurrentlySecure()) ? true : null)); // media_url should only exist if we need to re-load images
                        } elseif ($noimgUrl) 
                        {
                ...
                $html .= '<script type="text/javascript"> confData = new AmConfigurableData(' . Zend_Json::encode($confData) . ');
                                                              confData.textNotAvailable = "' . $this->__('Choose first option please...') . '";
                                                              confData.mediaUrlMain = "' . $this->getUrl('amconf/media', array('id' => $this->getProduct()->getId(), '_secure' => (Mage::app()->getStore()->isCurrentlySecure()) ? true : null)) . '";
