...
class Amasty_Xcoupon_Model_Mysql4_Report_Collection extends Mage_Sales_Model_Mysql4_Order_Collection
{
    public function addFields()
    {
        ...
        // modern versions
            ...
            if (version_compare(Mage::getVersion(), '1.9') > 0 && version_compare(Mage::getVersion(), '1.11') < 0){
                $field = 'number';
            }
            ...
            ;   
             $this->addFieldToFilter('sales_flat_order_item.product_type', array('eq'=>'simple'));
              $select->joinLeft('sales_flat_order_item', 'main_table.entity_id = sales_flat_order_item.order_id ',
                    array('product_names'=> new Zend_Db_Expr('GROUP_CONCAT(name SEPARATOR ",")')))
                ->group('main_table.entity_id');
