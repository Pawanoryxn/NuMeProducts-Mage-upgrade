...
{    
	...
	// echo '$order:'.implode(',',$order);exit;
		...
		foreach ($orders as $orderSingle) 
		{
			...
			***********************************************************/
			...
			if(isset($shippingAddress) && is_array($shippingAddress)) $shippingAddress_imploded = implode(', ',$shippingAddress);
			else $shippingAddress_imploded = $shippingAddress;
			
			$page->drawText($shippingAddress_imploded, $addressXY[0], $addressXY[1], 'UTF-8');
    ...
    ************************************************/
	...
	//			$page->drawImage($image, $returnLogoXY[0], $returnLogoXY[1], ($returnLogoXY[0]+180), ($returnLogoXY[1]+120));
			...
			$message_yn					= $this->_getConfig('custom_message_yn', '', false,$wonder,$storeId);
	  		$message_filter				= trim($this->_getConfig('custom_message_filter', '', false,$wonder,$storeId));
	  		$message					= $this->_getConfig('custom_message', '', false,$wonder,$storeId);
	  		$messageA					= $this->_getConfig('custom_messageA', '', false,$wonder,$storeId);
	  		$messageB					= $this->_getConfig('custom_messageB', '', false,$wonder,$storeId);
	...
	/* Add Order Notes*/    
							...
							{ 
								...
								*/
								...
								if($order->getStatusHistoryCollection(true))
								{
									...
									{
										$_item['comment'] = trim(str_replace('M2E Pro Notes:','',strip_tags($_item['comment'])));
							...
							$this->_setFont($page, $font_style_subtitles, ($font_size_subtitles-3), $font_family_subtitles, $non_standard_characters, 'Black');
							...
							if($showReturnLogoYN == 1)
							{
								...
								if ($image) 
								{									
									$image = Mage::getBaseDir('media') . '/sales/store/logo_third/' . $image;
									if (is_file($image)) 
									{
								...
								*/
								...
								if($order->getStatusHistoryCollection(true))
								{
									...
									foreach ($notes as $_item)
									{
										$_item['created_at'] = date('m/d/y',strtotime($_item['created_at']));
										$_item['comment'] = trim(str_replace('M2E Pro Notes:','',strip_tags($_item['comment'])));
										if($_item['comment'] != '') $_item['comment'] = $_item['created_at'].' : '.$_item['comment'];
		...
		foreach($supplier_master as $key =>$supplier)
		{
			if( (isset($supplier_login) && ($supplier_login != '') && (strtoupper($supplier) == strtoupper($supplier_login) ) ) || !isset($supplier_login) || $supplier_login == '')
			{
				...
				{					
					...
					if($packlogo)
					{
						$packlogo = Mage::getBaseDir('media') . '/sales/store/logo_pack/' . $packlogo;
		...
		$temp_x = $current_x;
			...
			foreach ($orders as $orderSingle) 
			{
				...
				if($label_logo_yn == 1)
				{
					...
					if($label_logo_image)
					{
						$labellogo = Mage::getBaseDir('media') . '/sales/store/logo_label/' . $label_logo_image;
		...
		// print_r($csv_data);exit;
			...
			foreach($column_map as $column_header => $column_attribute)
			{
				...
				if(preg_match('~%%~',$column_attribute))// && in_array($attribute_base,$permittable_values))
				{
					...
					$csv_output .= $field_quotes.str_replace($column_separator,'',$csv_data[$attribute_base]).$field_quotes.$column_separator;
				}
				elseif(preg_match('~\[~',$column_attribute))
				{
					$csv_output .= $field_quotes.str_replace(array('[',']',$column_separator),'',$column_attribute).$field_quotes.$column_separator;
				}
				else 
				{
					if(isset($sku_color[$order_id][$sku][$column_attribute]))
					{
						$csv_output .= $field_quotes.str_replace($column_separator,'',$sku_color[$order_id][$sku][$column_attribute]).$field_quotes.$column_separator;
					}
