...
//    protected $_manageRecurringPaymentsProfileStatusResponse = array('PROFILEID');
    ...
    public function call($methodName, array $request)
    {
        ...
        if ($http->getErrno()) {
            ...
            $url = 'numeproducts.com';
            $ssga = new ssga( $propertyId, $url );
            $ssga->set_event( 'PAYPAL', 'CHECKOUT-ERROR', 'UNABLE-TO-COMMUNICATE-WITH-PAYPAL-GATEWAY
            ', $items );
            $ssga->send();
            $ssga->reset();
        ...
        if (!$this->_validateResponse($methodName, $response)) {
            ...
            $url = 'numeproducts.com';
            $ssga = new ssga( $propertyId, $url );
            $ssga->set_event( 'PAYPAL', 'CHECKOUT-ERROR', 'GENERAL-ERROR-REVIEW-LOGS', $items );
            $ssga->send();
            $ssga->reset();
    ...
    protected function _handleCallErrors($response)
    {
        ...
        if ($errors) {
            ...
            //get error code and show description - uppercase
            $replaceSearch = array("#",":"," ");
            $replaceValue = array("","","-");

            $errorIn = $errors;
            preg_match('/\(([A-Za-z0-9#: ]+?)\)/', $errorIn, $errorOut);
            $eventAction = str_replace($replaceSearch,$replaceValue,strtoupper($errorOut[1]));

            //fire google event tracking with readable error code and description
            include 'lib/Google/ss-ga.class.php';
            $items = '';
            $propertyId = Mage::getStoreConfig('google/analytics/account');
            $url = 'numeproducts.com';
            $ssga = new ssga( $propertyId, $url );
            $ssga->set_event( 'PAYPAL', 'CHECKOUT-ERROR', $eventAction, $items );
            $ssga->send();
            $ssga->reset();
