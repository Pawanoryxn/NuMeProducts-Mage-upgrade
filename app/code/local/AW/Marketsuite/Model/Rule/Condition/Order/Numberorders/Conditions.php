...
class AW_Marketsuite_Model_Rule_Condition_Order_Numberorders_Conditions extends Mage_Rule_Model_Condition_Abstract
{
    ...
    {
        parent::__construct();
        $this->setType('marketsuite/rule_condition_order_purchasedquantity_conditions')->setValue(null);
    }
    ...
    public function getQuery($query)
    {
        ...
        switch ($op) {
            case '==':
                $filter = array('eq' => $this->getValueParsed());
                break;
            case '!=':
                $filter = array('neq' => $this->getValueParsed());
                break;
            case '>=':
                $filter = array('gteq' => $this->getValueParsed());
                break;
            case '<=':
                $filter = array('lteq' => $this->getValueParsed());
                break;
            case '>':
                $filter = array('gt' => $this->getValueParsed());
                break;
            case '<':
                $filter = array('lt' => $this->getValueParsed());
                break;
            case '{}':
                $filter = array('like' => $this->getValueParsed());
                break;
            case '!{}':
                $filter = array('nlike' => $this->getValueParsed());
                break;
            case '()':
                $filter = array('in' => $this->getValueParsed());
                break;
            case '!()':
                $filter = array('nin' => $this->getValueParsed());
                break;
        }
        return $query->addFieldToFilter($field, $filter);
    }
