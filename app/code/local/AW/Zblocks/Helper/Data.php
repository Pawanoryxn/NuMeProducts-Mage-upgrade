...
class AW_Zblocks_Helper_Data extends Mage_Core_Helper_Abstract
{
    ...
    protected function _isValid($data)
    {
        ...
        if ($currentProduct) {
            ...
            if (isset($conditions['conditions'])) {
                ...
                $conditions = $model->getConditions()->getConditions();
    ...
    public function getBlocks($customPosition, $blockPosition, $categoryPath, $currentCategoryId)
    {
        ...
        $collection
            ->addFieldToFilter('block_is_active', array('eq' => '1'))
        ...
        if (in_array($blockPosition, $needCategoryPositions) || $customPosition) {
            ...
            if (count($categoryPath)) { // if it's category or product page
                ...
                $select->where(new Zend_DB_Expr("(($a) OR ($b))" . $customPositionCondition));
            } else {
                $collection->getSelect()->where(new Zend_DB_Expr("category_ids=''" . $customPositionCondition));
            }
        }
