...
$installer->run("
    ...
    INSERT INTO $configTableName VALUES('" . AW_Followupemail_Model_Config::LAST_EXEC_TIME_DAILY . "', '"
        . ((int)$lastExecTime - 3600) . "');
...
foreach ($oldRules as $rule) {
    ...
    foreach (unserialize($rule['CATEGORIES']) as $c) {
        if (!$c) {
            ...
            break;
        } elseif ($k = array_search($c, $ruleNewCategories)) {
            unset($ruleNewCategories[$k]);
        }
    }

    $installer->run(
        'INSERT INTO ' . $ruleTableName . ' SET '
        ...
        . ', product_type_ids="' . ($rule['PRODUCT_TYPE'] ? $rule['PRODUCT_TYPE']
            : AW_Followupemail_Model_Source_Product_Types::PRODUCT_TYPE_ALL) . '"'
