...
class AW_Followupemail_IndexController extends Mage_Core_Controller_Front_Action
{
    ...
    public function resumeAction()
    {
        // testing code
       /*$datasMONEY = 10;
        Mage::getSingleton('core/session')->setspecialdiscountPer($datasMONEY);
        $xfcouponcode = str_replace('/','',$this->getRequest()->getParam('xfcouponcode')); 
        
         if($xfcouponcode)
        {
            Mage::getSingleton('core/session')->setxfcouponcode($xfcouponcode);
        }
        $url = "http://local.numeproducts.com/checkout/cart/";*/
        ?>
 <!-- <script type="text/javascript">window.location='http://local.numeproducts.com/checkout/cart/';</script>-->
        <?php
         // exit;
        // testing code
        if ($code = $this->getRequest()->getParam('code')) {
			...
			// print_r(($rule->getchain()));
			if ($queue = Mage::getModel('followupemail/queue')->loadByCode($code)) {
				$countNumberEmail = $queue['sequence_number'];
            }else
			$countNumberEmail = 1;

			$getpercentagte = unserialize($rule->getchain());
			$datas = $getpercentagte[$countNumberEmail-1];
                                    // print_r($datas);
                                    // $datas['MONEY']
			Mage::getSingleton('core/session')->setspecialdiscountPer($datas['MONEY']);
                                    $xfcouponcode = str_replace('/','',$this->getRequest()->getParam('xfcouponcode'));
                                    ...
                                    if($xfcouponcode = $this->getRequest()->getParam('xfcouponcode'))
                                    {
                                        Mage::getSingleton('core/session')->setxfcouponcode($xfcouponcode);
                                    }
