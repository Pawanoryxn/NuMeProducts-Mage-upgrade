        ...
        if (!empty($data) || $ruleId == 0) {
            ...
            if (Mage::helper('followupemail')->canUseAN()) {
                if (!isset($data['anl_segments'])
                    || !$data['anl_segments']
                    || !count(
                        is_array($data['anl_segments'])
                            ? $data['anl_segments']
                            : ($data['anl_segments'] = explode(',', $data['anl_segments']))
                    )
                    || (in_array(
                            AW_Followupemail_Model_Mysql4_Rule::ADVANCED_NEWSLETTER_SEGMENTS_ALL, $data['anl_segments']
                        )
            ...
            if (!isset($data['sale_amount_condition'])
                ...
                && is_array(
                    $saleAmount = explode(
                        AW_Followupemail_Model_Source_Rule_Saleamount::CONDITION_SEPARATOR, $data['sale_amount'], 2
                    )
                )
                && count($saleAmount) > 1
            ) {
                $data['sale_amount_condition'] = array_search(
                    $saleAmount[0], AW_Followupemail_Model_Source_Rule_Saleamount::getConditions()
                );
    ...
    public function saveAction($sendTest = false)
    {
        ...
        if ($data = $this->getRequest()->getPost()) {
            ...
            if ($data['sale_amount_value'] || $data['sale_amount_condition']) {
                $data['sale_amount']
                    = AW_Followupemail_Model_Source_Rule_Saleamount::getCondition($data['sale_amount_condition'])
                    . AW_Followupemail_Model_Source_Rule_Saleamount::CONDITION_SEPARATOR . $data['sale_amount_value'];
            } else {
                $data['sale_amount'] = '';
            }
