...
class AW_Followupemail_Adminhtml_QueueController extends Mage_Adminhtml_Controller_Action
{
    ...
    private function deleteEmail($id, $showMessage = true)
    {
        ...
        $message = "email id=$id deleted by Administrator";
        $subject = "Email deleted by Administrator";
    ...
    private function cancelEmail($id, $showMessage = true)
    {
        ...
        $message = "email id=$id cancelled by Administrator";
        $subject = "Email cancelled by Administrator";
        Mage::getSingleton('followupemail/log')->logSuccess($message, $this, $subject);
        if ($showMessage) {
    ...
    private function sendEmail($id, $showMessage = true)
    {
        ...
        if ($result === true) {
            $message = "email id=$id sent by Administrator";
            $subject = "Email sent by Administrator";
            Mage::getSingleton('followupemail/log')->logSuccess($message, $this, $subject);
            if ($showMessage) {
            ...
            $message = "email id=$id could not be sent by Administrator";
            $subject = "email could not be sent by Administrator";
    ...
    public function previewAction()
    {
        if ($id = $this->getRequest()->getParam('id')) {
            ...
            if (!isset($from['email_send_to_customer'])) {
                $from['email_send_to_customer'] = true;
            }
            ...
            $this->getResponse()->setBody(
                $this->getLayout()->createBlock('core/template')
                    ...
                    ->setTemplateStyles($email->getTemplateStyles())
    ...
    public function deleteAction()
    {
        if ($id = $this->getRequest()->getParam('id')) {
            try {
                ...
                $message = "email id=$id could not be deleted by Administrator";
                $subject = "email could not be deleted by Administrator";
                Mage::getSingleton('followupemail/log')->logError($message, $this, $subject);
            }
        }
        $this->_redirect('*/*/');
    ...
    public function cancelAction()
    {
        if ($id = $this->getRequest()->getParam('id')) {
            try {
                ...
                Mage::getSingleton('adminhtml/session')->addError($e->getMessage());
                $message = "email id=$id could not be cancelled by Administrator";
                $subject = "email could not be cancelled by Administrator";
                Mage::getSingleton('followupemail/log')->logError($message, $this, $subject);
            }
        }
        $this->_redirect('*/*/');
