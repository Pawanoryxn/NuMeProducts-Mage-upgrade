...
class AW_Followupemail_Helper_Data extends Mage_Core_Helper_Abstract
{
    ...
    public static function getCustomSMTPSettings()
    {
        ...
        if (array_key_exists('AW_Customsmtp', $_modules) && 'true' == (string)$_modules['AW_Customsmtp']->active
            ...
            if (!empty($needSSL)) {
                $customSMTPSettings['ssl'] = strtolower(
                    Mage::getStoreConfig(AW_Customsmtp_Helper_Config::XML_PATH_SMTP_SSL)
                );
            }
    ...
    public static function isMSSInstalled()
    {
        ...
        && !Mage::getStoreConfig('advanced/modules_disable_output/AW_Marketsuite');
    }
    ...
    public static function isWBTABInstalled()
    {
        ...
        && !Mage::getStoreConfig('advanced/modules_disable_output/AW_Relatedproducts');
    }
    ...
    */
    public static function isARP2Installed()
    {
        ...
        && !Mage::getStoreConfig('advanced/modules_disable_output/AW_Autorelated');
    }
    ...
    public function canUseAN()
    {
        if ($this->_canUseAN === null) {
            ...
            $this->_canUseAN = $_isInstalled
                && version_compare(
                    $modules['AW_Advancednewsletter']->version, self::FUE_AN_MINVERSION, '>='
                );
        }
    ...
    public function getGaConfig($rule)
    {
        ...
        if ($rule->getData('ga_term') != '') {
            $ga_params .= '&utm_term=' . rawurlencode($rule->getData('ga_term'));
        }
        if ($rule->getData('ga_content') != '') {
            $ga_params .= '&utm_content=' . rawurlencode(
                    $rule->getData('ga_content')
                );
        }
        return $ga_params;
    }
