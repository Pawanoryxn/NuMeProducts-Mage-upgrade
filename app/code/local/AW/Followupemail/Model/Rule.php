...
class AW_Followupemail_Model_Rule extends Mage_Core_Model_Abstract
{
    ...
    protected function _validateByCustomerGroup($customer)
    {
        ...
        if ($customer) {
            ...
            AW_Followupemail_Model_Source_Customer_Group::CUSTOMER_GROUP_NOT_REGISTERED, $this->_customerGroups
        )
        ) {
    ...
    protected function _checkSaleAmount($params, $source)
    {
        ...
        switch (array_search(
            $condition, AW_Followupemail_Model_Source_Rule_Saleamount::getConditions()
        )) // switch($condition)
        {
    ...
    public function validateByProduct($params)
    {
        ...
        if (count($this->_productTypes)
            && !in_array(
                AW_Followupemail_Model_Source_Product_Types::PRODUCT_TYPE_ALL, $this->_productTypes
            )
            && isset($params['product_type_ids'])
            && count($params['product_type_ids'])
            && count($this->_sku)
    ...
    protected function _getTemplate($templateId, $storeId = null)
    {
        ...
        $templateName = substr(
            $templateId,
            false !== (
            $pos = strpos($templateId, AW_Followupemail_Model_Source_Rule_Template::TEMPLATE_SOURCE_SEPARATOR))
                ? $pos + 1 : 0
        );
    ...
    protected function _createCustomer($objects)
    {
        if (!isset($objects['customer'])) {
            if (isset($objects['customer_id']) && $objects['customer_id']) {
                ...
                $billingAddress = AW_Followupemail_Helper_Data::getOrderAddress($objects['order'], 'billing');
    ...
    */
            ...
            foreach ($chain as $chainItem) {
                ...
                if ($this->getCouponEnabled()) {
                    ...
                    if (preg_match_all($pattern2, $emailTemplateContent, $matches) > 0) {
                        ...
                        $message = 'New coupon ' . $coupon->getCouponCode() . ' is created {' . print_r(
                                $coupon->getData(), true
                            ) . '}';
    ...
    public function processBirthday($params, $templateId, $timeDelay, $sequenceNumber)
    {
        ...
        if ($this->_isValid) {
            ...
            if ($this->getCouponEnabled()) {
                ...
                if (preg_match_all($pattern2, $emailTemplateContent, $matches) > 0) {
                    $coupon = Mage::helper('followupemail/coupon')->createNew($this);
                    $message
                        = 'New coupon ' . $coupon->getCouponCode() . ' is created {' . print_r($coupon->getData(), true)
                        . '}';
                    $subject = "New coupon is created";
                    Mage::getSingleton('followupemail/log')->logSuccess($message, $this, $subject);
        ...
        Mage::getSingleton('followupemail/log')->logWarning(
            ...
            . AW_Followupemail_Helper_Data::printParams($params), $this
        );
        return false;
    }
    ...
    */
    ...
    {
        if (strpos($eventType, AW_Followupemail_Model_Source_Rule_Types::RULE_TYPE_ORDER_STATUS_PREFIX) > -1) {
