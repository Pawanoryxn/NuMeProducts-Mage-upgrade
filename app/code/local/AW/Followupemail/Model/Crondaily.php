...
class AW_Followupemail_Model_Crondaily
{
    ...
    public function cronJobs()
    {
        ...
        if (self::$_isRunning
            ...
            if (AW_Followupemail_Model_Cron::DEBUG_MODE) {
                AW_Followupemail_Model_Log::log(
                    'CD Already running or minimal interval isn\'t match ' . date('d.m.Y H:i:s', $lastExecTime) . ' - '
                    . ($this->cronMinInterval - $now - $lastExecTime)
                );
            }
            return;
        }
        ...
        try {
            ...
            AW_Followupemail_Model_Log::log(
                'cron daily started, last execution time is '
                . date(
    ...
    protected function _checkBirthdays()
    {
        ...
        $customerDateOfBirthAttributeId = Mage::getModel('eav/entity_attribute')->loadByCode(
            $customerEntityTypeID, 'dob'
        )->getId();
        ...
        foreach ($ruleIds as $ruleId) {
            ...
            foreach (unserialize($rule->getChain()) as $chain) {
                $select = $read->select()
                    ...
                    ->where(
                        'DATE_FORMAT(dob.value, "%m-%d")=?',
                        date('m-d', $time - (int)($chain['BEFORE'] * $chain['DAYS'] * 86400))
                    );
                ...
                if (count($birthDays)) {
                    ...
                    foreach ($birthDays as $birthDay) {
                        ...
                        AW_Followupemail_Model_Log::log(
                            'customer birthday event processing, rule_id=' . $ruleId . ', customerId='
                            . $params['customer_id'] . ', dob=' . $params['dob'] . ', store_id=' . $params['store_id']
                        );

                        $rule->processBirthday(
                            $params, $chain['TEMPLATE_ID'], $chain['BEFORE'] * $chain['DAYS'], $sequenceNumber
                        );
                    }
                }
                $sequenceNumber++;
