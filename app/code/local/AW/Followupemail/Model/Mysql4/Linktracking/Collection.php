...
class AW_Followupemail_Model_Mysql4_Linktracking_Collection extends Mage_Core_Model_Mysql4_Collection_Abstract
{
    ...
    public function getLinktrackingCollection($queryType, $fields = false)
    {
        if (!$fields) {
            ...
            $fields = array_merge(self::$fields, is_array($fields) ? $fields : array($fields));
        }
        ...
        $this->getSelect()
            ...
            ->joinInner(
                array('r' => $this->getTable('followupemail/rule')),
                'e.rule_id=r.id', ''
            );
        ...
        switch ($queryType) {
            case AW_Followupemail_Model_Source_Linktracking_Types::LINKTRACKING_TYPE_LINK_CART :
                $this->getSelect()
                    ...
                    ->where(
                        'r.event_type="' . AW_Followupemail_Model_Source_Rule_Types::RULE_TYPE_ABANDONED_CART_NEW . '"'
                    )
                    ->where('q.is_active=1');
                break;
