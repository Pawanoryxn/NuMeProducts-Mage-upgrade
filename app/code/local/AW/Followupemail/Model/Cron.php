...
class AW_Followupemail_Model_Cron
{
     ...
     */
    const CACHE_LOCK_ID = 'aw_fue_lock';
    ...
    public function cronJobs()
    {
        ...
        $this->_lastExecTimeMySQL = date(
            AW_Followupemail_Model_Mysql4_Queue::MYSQL_DATETIME_FORMAT, $this->_lastExecTime
        );
        ...
        try {
            ...
            AW_Followupemail_Model_Log::log(
                'cron started, last execution time is '
                . date(
                    AW_Followupemail_Model_Mysql4_Queue::MYSQL_DATETIME_FORMAT,
        ...
        $timeShift = Mage::app()->getLocale()->date()->get(Zend_Date::TIMEZONE_SECS);
        AW_Followupemail_Model_Log::log(
            'cron stopped at ' . date(AW_Followupemail_Model_Mysql4_Queue::MYSQL_DATETIME_FORMAT, time() + $timeShift) .
            '. Last time is ' . date(
                AW_Followupemail_Model_Mysql4_Queue::MYSQL_DATETIME_FORMAT, $this->_now + $timeShift
            )
        );
    }
    ...
    protected function _removeOldCoupons()
    {
        ...
        $collection->getSelect()->joinLeft(
            array('scr' => $collection->getTable('salesrule/rule')), 'main_table.rule_id = scr.rule_id'
        )
        ...
        if (self::DEBUG_MODE) {
            AW_Followupemail_Model_Log::log(
                sprintf("Total %d old coupons found", $collection->getSize())
            );
        }
    ...
    protected function _checkOrderStatusHistory()
    {
        ...
        while ($currentPage <= $_pages) {
            if (self::DEBUG_MODE) {
                AW_Followupemail_Model_Log::log(
                    "Processing page {$currentPage} from {$_pages} total"
                );
            }
            ...
            if ($lastId) {
                ...
                $statusHistoryCollection->addAttributeToFilter(
                    'main_table.created_at', array('gt' => $this->_lastExecTimeMySQL)
                );
            }
            ...
            $select = $read->select()
                ...
                ->joinInner(
                    array('o' => $resource->getTableName('sales/order')), 'o.increment_id=q.reserved_order_id', ''
                )
                ->columns('qio.value')
            ...
            foreach ($statusHistoryCollection->getItems() as $historyItem) {
                $eventName
                    =
                    AW_Followupemail_Model_Source_Rule_Types::RULE_TYPE_ORDER_STATUS_PREFIX . $historyItem->getStatus();

                $order->reset()->load($historyItem->getParentId());
                if (
                    ($eventName == AW_Followupemail_Model_Source_Rule_Types::RULE_TYPE_ORDER_STATUS_PREFIX . 'pending') ||
                    ($eventName == AW_Followupemail_Model_Source_Rule_Types::RULE_TYPE_ORDER_STATUS_PREFIX . 'pending_payment')
                ) {
                    $_ruleNewCustomer = $dbReader->getRuleIdsByEventType(
                        AW_Followupemail_Model_Source_Rule_Types::RULE_TYPE_CUSTOMER_NEW
                    );
                    if (!empty($_ruleNewCustomer)) {
                        $queue->cancelByEvent(
                            $order->getCustomerEmail(),
                            AW_Followupemail_Model_Source_Rule_Types::CANCEL_TYPE_CUSTOMER_PLACED_NEW_ORDER,
                            $order->getCustomerId()
                        );
                        $queue->cancelByEvent($order->getCustomerEmail(), $eventName, $order->getCustomerId());
                    }
                    $queue->cancelByEvent(
                        $order->getCustomerEmail(),
                        AW_Followupemail_Model_Source_Rule_Types::CANCEL_TYPE_CUSTOMER_PLACED_NEW_ORDER, false
                    );
                }
                ...
                foreach ($ruleIds as $key => $value) {
                    if ($dbReader->isOrderStatusProcessed($historyItem->getParentId(), $value)) {
                        AW_Followupemail_Model_Log::log(
                            'order status duplicated, orderId=' . $historyItem->getParentId() . ' status='
                            . $historyItem->getStatus() . ' ruleId=' . $value
                        );
                        unset($ruleIds[$key]);
                    }
                }
                ...
                if (!empty($ruleIds)) {
                    ...
                    $params['category_ids'] = Mage::helper('followupemail')->noEmptyValues(
                    ...
                    foreach ($ruleIds as $ruleId) {
                        AW_Followupemail_Model_Log::log(
                            "order status orderId={$order->getId()} validating, ruleId=$ruleId"
                        );
    ...
    protected function _checkAbandonedCarts()
    {
        ...
        $select = $read->select()
            ...
            ->joinInner(
                array('i' => $resource->getTableName('sales/quote_item')), 'q.entity_id=i.quote_id', array(
        ...
        foreach ($carts as $cart) {
            $queue->cancelByEvent(
                $cart['customer_email'], AW_Followupemail_Model_Source_Rule_Types::RULE_TYPE_ABANDONED_CART_NEW,
                $cart['quote_id']
            );
        }

        $ruleIds = Mage::getModel('followupemail/mysql4_rule')->getRuleIdsByEventType(
            AW_Followupemail_Model_Source_Rule_Types::RULE_TYPE_ABANDONED_CART_NEW
        );
        if (!count($ruleIds)) {
            return;
        }
    ...
    protected function _checkCustomerLastActivity()
    {
        ...
        if (Mage::helper('followupemail')->checkExtensionVersion('Mage_Log', '0.7.7', '<=')) {
            $collection->getSelect()->from(
                array('main_table' => $resource->getTableName('log/visitor')),
        ...
        $collection->getSelect()
            ->join(
                array('c' => $resource->getTableName('log/customer')),
            ...
            ->joinLeft(
                array('u' => $resource->getTableName('log/url_info_table')),
                'main_table.last_url_id=u.url_id',
                'url'
            )
            ->group('main_table.visitor_id');

        $this->_processCustomerActivity(
            $collection, AW_Followupemail_Model_Source_Rule_Types::RULE_TYPE_CUSTOMER_LAST_ACTIVITY
        );
    }
    ...
    protected function _checkCustomerLogin()
    {
        ...
        $this->_processCustomerActivity(
            $collection, AW_Followupemail_Model_Source_Rule_Types::RULE_TYPE_CUSTOMER_LOGGED_IN
        );
    }
    ...
    protected function _processCustomerActivity($collection, $ruleType)
    {
        ...
        foreach ($collection as $visit) {
            ...
            foreach ($ruleIds as $ruleId) {
                AW_Followupemail_Model_Log::log(
                    'customer last activity' . " customerId={$params['customer_id']} validating, ruleId=$ruleId"
                );
                Mage::getModel('followupemail/rule')->load($ruleId)->process($params, $objects);
            }
        }
