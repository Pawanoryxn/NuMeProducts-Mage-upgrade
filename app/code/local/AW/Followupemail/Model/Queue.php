...
class AW_Followupemail_Model_Queue extends Mage_Core_Model_Abstract
{
     ...
     */
    public function add(
        $code, $sequenceNumber, $senderName, $senderEmail, $recipientName, $recipientEmail, $ruleId, $scheduledAt,
        $subject, $content, $objectId, $params, $styles = ''
    ) {
        ...
        $message = "email added id={$this->getId(
        )} seq.No=$sequenceNumber email=$recipientEmail name=$recipientName ruleId=$ruleId objectId=$objectId code=$code scheduledAt=$scheduledAt GMT subject=$subject";
    ...
    public function send()
    {
        ...
        $email->setTemplateStyles($this->getTemplateStyles());
        ...
        if ($result) {
            ...
            $message = 'email id=' . $this->getId() . ' sent OK';
            $subject = "E-mail sent OK";
            Mage::getSingleton('followupemail/log')->logSuccess($message, $this, $subject);
        } else {
            $this->setStatus('F')
                ->setSentAt(date(AW_Followupemail_Model_Mysql4_Queue::MYSQL_DATETIME_FORMAT, time()))
            ...
            $message = 'email id=' . $this->getId() . ' sending error';
            $subject = "E-mail sending error";
