...
class AW_Followupemail_Model_Events
{
    ...
    */
    public function wishlistProductAdd($eventData)
    {
        ...
        /** @var $queue AW_Followupemail_Model_Mysql4_Queue */
    ...
    {
        if ($param instanceof AW_Followupemail_Model_Queue) {
            $queue = $param;
        } else {
            $queue = Mage::getResourceModel('followupemail/queue')->load($param);
        }

        $queueResource = $queue->getResource();
        $queueResource->cancelByEvent(
            $queueResource->getEmailByCustomerId($queue->getRecipientEmail()) ? $queueResource->getEmailByCustomerId(
        ...
        if ($customer->getData('group_id') != $customer->getOrigData('group_id')
            ...
            if (count($ruleIds)) {
                $params = array(
                    ...
                    'customer_new_group' => Mage::getModel('customer/group')->load($customer->getData('group_id'))
    ...
    public function addFUECouponType($event)
    {
        ...
        $couponTypes[Mage::helper('followupemail/coupon')->getFUECouponsCode()] = Mage::helper('followupemail')->__(
            'FUE Generated Coupons'
        );
    ...
    public function checkCouponsMenu($event)
    {
        ...
        if (isset($_t1) && $_t1 && isset($_t1->coupons)) {
            unset($_t1->coupons);
        }
        if (isset($_t2) && $_t2 && isset($_t2->coupons)) {
            unset($_t2->coupons);
        }
    }
    ...
    {
        if (Mage::registry(self::NEWSLETTER_SUBSCRIPTION_PROCESSED_FLAG)) {
            return;
        }
            ...
            if (self::isNewCustomer($customer) == true) {
                ...
                foreach ($ruleIds as $ruleId) {
                    AW_Followupemail_Model_Log::log(
                        'customer registered, customerId=' . $params['customer_id'] . " validating, ruleId=$ruleId"
                    );
                    Mage::getModel('followupemail/rule')->load($ruleId)->process($params, $objects);
                }
            }
