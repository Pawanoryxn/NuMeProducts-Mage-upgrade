...
/**
 * Class extended from salesrule Validator to get access to cached rules applied to quote
 * which stored in protected property
 */
class Oro_SalesRule_Model_Validator extends Mage_SalesRule_Model_Validator
{

    /**
     * Checks if given rule ID applicable to quote address
     *
     * @param int $ruleId
     * @param Mage_Sales_Model_Quote_Address $address
     * @return bool
     */
    public function validateRule($ruleId, $address)
    {
        $isValid = false;
        foreach ($this->_getRules() as $rule) {
            if ($rule->getId() == $ruleId) {
                $isValid = $this->_canProcessRule($rule, $address);
                break;
            }
        }
     ...
     * @param $rule
     * @return Oro_SalesRule_Model_Validator
     */
    public function processDiscountRule(Mage_Sales_Model_Quote_Item_Abstract $item, $rule)
    {
        $quote                  = $item->getQuote();
     ...
     * Forced init for rules that have discount conditions
     *
     * @param mixed $items
     * @param Mage_Sales_Model_Quote_Address $address
    ...
    public function initTotals($items, Mage_Sales_Model_Quote_Address $address)
    {
        ...
        foreach ($this->_getRules() as $rule) {
            if (Mage_SalesRule_Model_Rule::CART_FIXED_ACTION == $rule->getSimpleAction()
                ...
                || Mage::helper('oro_salesrule')->ruleHasDiscountCondition($rule)) {
