...
*/
class Atypicalbrands_Commercebug_Model_Collectorsysteminfo extends Atypicalbrands_Commercebug_Model_Observingcollector
{
    protected $_items;
    public function collectInformation($observer)
    {
        ...
        {
            Atypicalbrands_Commercebug_Model_Crossareaajax::validateControllers();
        }
        
        $system_info->ajax_paths = new stdClass;
        $system_info->form_key = $this->getShim()->getSingleton('core/session')->getFormKey();
        foreach(Atypicalbrands_Commercebug_Model_Crossareaajax::getAjaxActionNames() as $action)
        {
