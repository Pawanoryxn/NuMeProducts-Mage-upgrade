...
class Atypicalbrands_Commercebug_Helper_Formatlog_Allsimple extends Atypicalbrands_Commercebug_Helper_Data
{
    protected $_info=array();
    public function format($thing)
    {
        $object = $this->getShim()->getModel('commercebug/jsonbroker')->jsonDecode($thing);						
        ...
        if(isset($object->observers))
        {
            foreach($object->observers as $item)
            {
                foreach(get_object_vars($item) as $name=>$props)
                {
                    ...
                    Atypicalbrands_Commercebug_Model_Shim::Log('Mage2 refactoring needed at ' . __CLASS__ . ' ' . __LINE__);
                    unset($info['config']);
                    $result .= implode("\t", $info) . "\n";
                }                
