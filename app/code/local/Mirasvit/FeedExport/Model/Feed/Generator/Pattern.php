...
class Mirasvit_FeedExport_Model_Feed_Generator_Pattern extends Varien_Object
{
    ...
    public function parsePattern($pattern)
    {
        ...
        if (!isset(self::$_patterns[$pattern])) {
            $matches = array();
            preg_match('/{([^},|]+)([|])?(parent|grouped|configurable|bundle)?([^}]*)}/', $pattern, $matches);
            if (!isset($matches[1])) {
                return false;
            }
            $key  = $matches[1];
    ...
    public function getPatternValue($content, $scope = null, $obj = null)
    {
        ...
        foreach ($matches[0] as $pattern) {
            ...
            if ($this->hasData('value_preparer')) {
                $preparer = $this->getData('value_preparer');
                $value    = call_user_func($preparer, $value);
            }

            // mb_regex_encoding("UTF-8");
            // $value = mb_ereg_replace('[^(\x20-\x7F\)]*', '', $value);
            if (!is_object($value)) {
