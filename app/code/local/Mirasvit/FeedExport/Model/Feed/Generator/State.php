...
class Mirasvit_FeedExport_Model_Feed_Generator_State extends Varien_Object
{
    ...
    public function getEta($idx, $size, $time)
    {
        $etaMsg   = null;
        ...
        $msg = __("%d of %d (%01.1f%%)");
        $msg = sprintf($msg, $idx, $size, $idx / $size * 100);
    ...
    public function toHtml()
    {
        ...
        if (is_array($this->getChain())) {
            foreach ($this->getChain() as $item) {
                ...
                if ($item['status'] == 'processing') {
                    if (isset($item['idx'])
                        ...
                        $text .= '<span class="eta">'.$this->getEta($item['idx'], $item['size'], $item['started_at']).'</span>';
                    }
                }
            }
        }
            ...
            foreach ($this->getChain() as $item) {
                ...
                if ($item['status'] == self::CHAIN_STATUS_PROCESSING) {
                    ...
                    if (isset($item['idx'])
                        && isset($item['size'])
                        && isset($item['started_at'])) {
                        $text .= $this->getEta($item['idx'], $item['size'], $item['started_at']);
                    }
