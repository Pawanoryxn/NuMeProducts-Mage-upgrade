...
class Mirasvit_Seo_Model_Opengraph extends Mage_Core_Model_Abstract
{
    ...
    public function modifyHtmlResponse($e)
    {
        ...
        if (strpos(trim($body), '<!DOCTYPE html') !== 0 && strpos($body, '<html') !== 0) {
            return;
        }
        ...
        if ($product = Mage::registry('current_product')) {
            ...
            if ($config->isRichSnippetsEnabled()) {
                $tags[] = "<!-- mirasvit rich snippets begin -->";
                $tags[] = '<article itemscope itemtype="http://data-vocabulary.org/Product"/>';
                $tags[] = '<h2 itemprop="name" class="review-aggregate-hide">' . $product->getName() . '</h2>';
                $tags[] = "<!-- mirasvit rich snippets end -->";
            }
